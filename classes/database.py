...
class database:
    ...
    def start(self):
        ...
        sql_statement = "CREATE TABLE IF NOT EXISTS `accountimg` (`id` int NOT NULL,`img` mediumblob,`imgurl` varchar(255) GENERATED ALWAYS AS (concat(_utf8mb4'../../../../img/user_imgs/',`id`,_utf8mb4'.jpg')) STORED,PRIMARY KEY (`id`),CONSTRAINT `accountimg_id` FOREIGN KEY (`id`) REFERENCES `accounts` (`id`))"
        self.csr.execute(sql_statement)

        sql_statement = "CREATE TABLE IF NOT EXISTS `studentinfo` (`id` int NOT NULL,`program` enum('Computer Science','Entertainment and Multimedia Computing','Information System','Information Technology') NOT NULL,PRIMARY KEY (`id`),UNIQUE KEY `id_UNIQUE` (`id`),CONSTRAINT `studentinfo_id` FOREIGN KEY (`id`) REFERENCES `accounts` (`id`))"
        self.csr.execute(sql_statement)

        sql_statement = "CREATE TABLE IF NOT EXISTS `instructorinfo` (`id` int NOT NULL, `status` tinyint(1) DEFAULT '0', PRIMARY KEY (`id`), UNIQUE KEY `id` (`id`), CONSTRAINT `instructorinfo_id` FOREIGN KEY (`id`) REFERENCES `accounts` (`id`))"
        self.csr.execute(sql_statement)

        sql_statement = "CREATE TABLE IF NOT EXISTS `chairmans` (`programs` varchar(45) NOT NULL, `id` int DEFAULT NULL, PRIMARY KEY (`programs`), UNIQUE KEY `programs` (`programs`), KEY `chairmans_idx` (`id`), CONSTRAINT `chairmans_id` FOREIGN KEY (`id`) REFERENCES `accounts` (`id`))"
        self.csr.execute(sql_statement)
        ...
        sql_statement = "CREATE TABLE IF NOT EXISTS `instructorcourses` (`instructor` int NOT NULL, `course` varchar(255) NOT NULL, PRIMARY KEY (`instructor`,`course`), KEY `instructorcourses_course_idx` (`course`), CONSTRAINT `instructorcourses_course` FOREIGN KEY (`course`) REFERENCES `courses` (`id`), CONSTRAINT `instructorcourses_instructor` FOREIGN KEY (`instructor`) REFERENCES `accounts` (`id`))"
        self.csr.execute(sql_statement)

        sql_statement = "CREATE TABLE IF NOT EXISTS `number` (`number` varchar(45) NOT NULL DEFAULT '0', PRIMARY KEY (`number`))"
        self.csr.execute(sql_statement)

        sql_statement = "CREATE TABLE IF NOT EXISTS `enrollmentreport` (`id` int NOT NULL AUTO_INCREMENT, `queuenumber` varchar(45) NOT NULL, `date` varchar(45) NOT NULL, `time` varchar(45) NOT NULL, `status` enum('onQueue','done') NOT NULL DEFAULT 'onQueue', PRIMARY KEY (`id`), UNIQUE KEY `id` (`id`))"
        self.csr.execute(sql_statement)
    ...
    def setup_queue(self):
        ...
        self.csr.execute("INSERT INTO `number` (`number`) VALUES ('0');")
        self.conn.commit()
